    ...
    template <class T>
        class list
        {
            ...
            public:
                ...
                // 改变链表的大小为指定大小，并使用默认值填充新元素
                void resize(size_type new_size) { resize(new_size, value_type()); }

                // 改变链表的大小为指定大小，并使用给定值填充新元素
                void resize(size_type new_size, const value_type& value);

                // 与另一个链表交换内容
                void swap(list& rhs) noexcept
                {
                    mystl::swap(node_, rhs.node_); // 交换节点指针
                    mystl::swap(size_, rhs.size_); // 交换大小
                }

                // list 相关操作

                // 将另一个链表的所有元素插入到指定位置之前
                void splice(const_iterator pos, list& other);

                // 将另一个链表中的一个元素插入到指定位置之前
                void splice(const_iterator pos, list& other, const_iterator it);

                // 将另一个链表中的指定范围内的元素插入到指定位置之前
                ...
                // 移除链表中连续重复的元素，保留每个数值只出现一次
                void unique()
                { 
                    unique(mystl::equal_to<T>()); 
                }
                // 移除链表中满足给定二元谓词的连续重复的元素
                template <class BinaryPredicate>
                    void unique(BinaryPredicate pred);

                // 合并当前链表和给定链表，要求两个链表都已经有序
                void merge(list& x)
                { 
                    merge(x, mystl::less<T>()); 
                }
                // 合并当前链表和给定链表，使用给定的比较函数进行排序
