...
// 这份代码是一个函数对象头文件，定义了一系列函数对象和哈希函数。
// 函数对象是可调用对象，在STL中用于算法和容器等地方进行操作。这些函数对象包括数学运算、比较运算、逻辑运算等，可以方便地在代码中使用。
// 哈希函数对象，用于将不同类型的数据转换成哈希值。哈希函数在哈希表等数据结构中广泛使用，用于快速查找和插入元素。
...
#undef MYSTL_TRIVIAL_HASH_FCN
    ...
    {
        // 哈希函数使用 FNV-1 hash 算法
#if (_MSC_VER && _WIN64) || ((GNUC || clang) &&SIZEOF_POINTER == 8)
    ...
    template <>
        struct hash<long double>
        {
            ...
            {
                // 如果值为 0，则返回 0
                return val == 0.0L ? 0 : bitwise_hash((const unsigned char*)&val, sizeof(long double));
            }
        };
